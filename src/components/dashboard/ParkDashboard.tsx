...
import { getWaltDisneyWorldParks, getParkLiveData, getParkSchedule } from '@/lib/api/themeParks';
...
export default function ParkDashboard({
    ...
    const waitTimeStats = liveWaitTimes ? {
        ...
        longestName: sortedAttractions.operating.length > 0
            ...
            : '',
        belowThirty: sortedAttractions.operating.filter(a => a.waitTime <= 30).length,
        belowThirtyPercent: sortedAttractions.operating.length > 0
            ? Math.round((sortedAttractions.operating.filter(a => a.waitTime <= 30).length / sortedAttractions.operating.length) * 100)
            : 0,
    } : null;

    // Determine wait time trend indicator
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <div className="text-xs text-muted-foreground">
                                                {waitTimeStats.belowThirty} out of {sortedAttractions.operating.length} attractions
                                            </div>
                                        </div>
                                    </div>
                                )}
                    ...
                    <Card>
                        ...
                        <CardContent className="px-0 py-0">
                            <ScrollArea className="h-[260px]">
                                <div className="px-4">
                                    {sortedAttractions?.operating.slice(0, 10).map((attraction, index) => (
                                        <div
                                            key={attraction.id}
                                            className={cn(
                                                "py-2 flex items-center justify-between",
                                                index !== sortedAttractions.operating.slice(0, 10).length - 1 && "border-b"
                                            )}
                                        >
                                            <div className="flex items-center">
                                                <div
                                                    className={cn(
                                                        ...
                                                        attraction.waitTime <= 10 ? "bg-green-500" :
                                                            attraction.waitTime <= 30 ? "bg-blue-500" :
                                                                attraction.waitTime <= 60 ? "bg-amber-500" :
                                                                    "bg-red-500"
                                                    )}
                                                >
                                                    {index + 1}
                                        ...
                                        {showCrowdLevels && (
                                            <>
                                                ...
                                                <div>
                                                    ...
                                                    <div className="space-y-2">
                                                        {crowdLevelData.forecast.map((day, index) => (
